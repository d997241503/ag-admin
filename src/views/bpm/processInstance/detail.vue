	...
	<div class="app-container">
		...
		<el-card class="box-card" v-loading="processInstanceLoading" v-for="(item, index) in runningTasks" :key="index">
			...
			<el-col :span="16" :offset="6">
				...
				<!-- <parser :ref="'approvalForm' + index" :key="new Date().getTime()"
					:form-conf="item.approvalDetailForm" /> -->
				<k-form-build v-if="item.approvalDetailForm.list && item.approvalDetailForm.list.length" :ref="'approvalForm' + index" :disabled="item.disabled"
					:value="item.approvalDetailForm" />
				<div style="margin-left: 10%; margin-bottom: 20px; font-size: 14px;">
					<el-button icon="el-icon-edit-outline" type="success" size="mini" @click="handleAudit(item, true)">
						通过</el-button>
					<el-button icon="el-icon-circle-close" type="danger" size="mini" @click="handleAudit(item, false)">
						不通过</el-button>
					<el-button icon="el-icon-edit-outline" type="primary" size="mini"
						@click="handleUpdateAssignee(item)">转办</el-button>
					<el-button icon="el-icon-edit-outline" type="primary" size="mini" @click="handleDelegate(item)">委派
					</el-button>
					<el-button icon="el-icon-refresh-left" type="warning" size="mini" @click="handleBack(item)">退回
					</el-button>
				</div>
			</el-col>
		</el-card>
		...
		<el-card class="box-card" v-loading="processInstanceLoading">
			<div slot="header" class="clearfix">
				...
				<!-- <el-button style="float: right;" type="primary" @click="handleDetail">查看详情
					</el-button> -->
			</div>
			<el-col
				v-show="this.processInstance.processDefinition && this.processInstance.processDefinition.formType === 10"
				:span="16" :offset="6">
				<div>
					<!-- <parser :key="new Date().getTime()" :form-conf="detailForm" /> -->
					<k-form-build ref="kfbsq" :disabled="true" :value="jsonDataSq" />
				</div>
			</el-col>
			<div
				v-if="this.processInstance.processDefinition && this.processInstance.processDefinition.formType === 20">
				...
				<el-button type="primary" @click="handleDetail">点击查看</el-button>
			</div>
		</el-card>
		<!-- 审批记录 -->
					...
					<el-timeline>
						<el-timeline-item v-for="(item, index) in tasks" :key="index" :icon="getTimelineItemIcon(item)"
							...
							<el-card :body-style="{ padding: '10px' }">
								...
								<label
									style="color:#8a909c; font-weight: normal">{{ parseTime(item.createTime) }}</label>
								<label v-if="item.endTime" style="margin-left: 30px;font-weight: normal">审批时间：</label>
								<label v-if="item.endTime" style="color:#8a909c;font-weight: normal">
									{{ parseTime(item.endTime) }}</label>
								<label v-if="item.durationInMillis"
									style="margin-left: 30px;font-weight: normal">耗时：</label>
								<label v-if="item.durationInMillis" style="color:#8a909c;font-weight: normal">
									{{ getDateStar(item.durationInMillis) }} </label>
								<!-- <parser v-if="item.durationInMillis" style="margin-top: 20px;"
									:key="new Date().getTime()" :form-conf="item.approvalDetailForm" /> -->
								<k-form-build v-if="item.durationInMillis" style="margin-top: 20px;" ref="kfbxq" :disabled="true" :value="item.approvalDetailForm" />
								<p v-if="item.reason">
		...
		<el-dialog title="转派审批人" :visible.sync="updateAssignee.open" width="500px" append-to-body>
			<el-form ref="updateAssigneeForm" :model="updateAssignee.form" :rules="updateAssignee.rules"
				label-width="110px">
				<el-form-item label="新审批人" prop="assigneeUserId">
					...
					<el-select v-model="updateAssignee.form.assigneeUserId" filterable remote placeholder="请选择新审批人"
		...
		methods: {
			...
			getDetail() {
				...
				getProcessInstance(this.id).then(response => {
					...
					if (this.processInstance.processDefinition.formType === 10) {
						...
						Promise.all([
							this.processInstance.processDefinition.formFields.forEach(item => {
								this.jsonDataSq.list.push(JSON.parse(item));
							})
						]).then(() => {
							this.$nextTick(() => {
								setTimeout(() => {
									this.$refs.kfbsq.setData(this.processInstance
										.formVariables);
								}, 300)
							})
						})
				...
				getTaskListByProcessInstanceId(this.id).then(response => {
					...
					this.tasks.forEach((task, index) => {
						...
						this.tasks[index] = {
							...task,
							approvalDetailForm: {
								// ...JSON.parse(task.formConf),
								// fields: decodeFields(task.formFields)
								config: task.formConf ? JSON.parse(task.formConf) : undefined,
								list: list
							},
							disabled: true, // 表单禁用
							formBtns: false, // 按钮隐藏
						...
						// 设置表单的值
						// this.tasks[index].approvalDetailForm.fields.forEach(item => {
						// 	const val = task.formVariables ? task.formVariables[item.__vModel__] :
						// 		undefined
						// 	if (val) {
						// 		item.__config__.defaultValue = val
						// 	}
			...
			handleSelect(row) {
				...
				if (row.formId) {
					...
					this.jsonDataSq.config = JSON.parse(row.formConf);
			...
			handleAudit(task, pass) {
				...
				// let approvalFormData = this.$refs['approvalForm' + index][0].getFormData();
				let approvalFormData = undefined;
				if(this.$refs['approvalForm' + index]) {
					this.$refs['approvalForm' + index][0].getData().then(values => {
						approvalFormData = values;
						this.$refs['form' + index][0].validate(valid => {
							...
							const data = {
								...
								// variables: approvalFormData.values
								variables: approvalFormData
							}
							if (pass) {
