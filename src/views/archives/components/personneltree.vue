...
<script>
	...
	export default {
		...
		created() {
            this.orgId = this.$route.query.deptId;
			this.getList();
		},
		...
		methods: {
			...
			getList() {
				...
				listByOrg(this.orgId)
					.then(response => {
						if (response && response.status === 200) {
                            this.dataList = response.data
                            if(this.dataList.length) this.$emit('refreshData', this.dataList[0]);
			...
			checkedStatus(index) {
				...
				this.$refs[`leaderItem${index}`][0].style.color = 'rgb(188,0,0)';
			},
...
<style lang="scss" scoped>
	.custom-tree-container {
		...
		.leader-item:hover {
			// background-color: #4490FE !important;
