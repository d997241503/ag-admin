<template>
	<div class="app-container calendar-list-container nav-content">
		<div ref="searchSection" class="page-info" style="padding-bottom:0;margin-bottom: 20px;">
			<!-- <div v-if="integrityArchives_btn_inspection || integrityArchives_btn_first" class="search-info" :class="[(!integrityArchives_btn_inspection && !integrityArchives_btn_first)?'active':'otherActiveClass']">
				<div class="search-left">
					<el-form :inline="true">
						<el-form-item v-if="integrityArchives_btn_inspection">
							<el-select v-model="listQuery.violationCategory" class="filter-item" placeholder="请选择"
							style="width: 300px;">
								<el-option v-for="item in outsideOptions" :key="item.value" :label="item.label" :value="item.value">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item v-else>
							<el-select v-model="assignment" class="filter-item" placeholder="请选择任务" @change="assignmentChange"
							style="width: 200px;">
				...
				<div class="bu-right" v-if="integrityArchives_btn_inspection">
					...
					<el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_history" class="filter-item" type="text" v-waves icon="search" @click="historyHandle()">固化任务历史记录</el-button>
				</div>
				<div class="bu-right" v-else>
					<el-button class="filter-item" style="margin-left: 10px;" @click="solidificationPutHandle('oneself')"
						type="primary" icon="edit">+提报其他表单</el-button>
				</div>
			</div> -->
			 <div v-if="!integrityArchives_btn_inspection || skipVisible" class="radio-group">
                <el-radio-group class="radio-group-button" v-model="tabPosition" style="margin-bottom: 30px;" @change="radioChange">
                    <el-radio-button label="我的提报">我的提报</el-radio-button>
                    <el-radio-button label="我的审核">我的审核</el-radio-button>
                    <el-radio-button label="我的查看">我的查看</el-radio-button>
                </el-radio-group>
            </div>
			...
			<div class="search-info" style="margin-bottom: 0px;">
				<div class="search-left">
					<el-form :inline="true" :model="listQuery" @keyup.enter.native="getList()" ref="queryForm">
						<!-- <el-form-item v-if="tabPosition !== '我的查看'">
						...
						<el-button v-if="tabPosition == '我的提报' && integrityArchives_btn_putforward && (!integrityArchives_btn_inspection || skipVisible)" class="filter-item" style="margin-left: 10px;" @click="solidificationPutHandle('add', 'oneself')"
						...
						<el-button v-if="tabPosition == '我的审核' && (!integrityArchives_btn_inspection || skipVisible)" class="filter-item" style="margin-left: 10px;" @click="handleComments"
							type="primary" icon="edit" :key="tabPosition" :disabled="!multipleSelection.length">批量审核</el-button>
						<!-- <el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_solidification" class="filter-item" style="margin-left: 10px;" @click="solidificationHandle"
						type="primary" icon="edit">固化任务设置</el-button>
						<el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_temporary" class="filter-item" style="margin-left: 10px;" @click="temporaryHandle"
						type="primary" icon="edit">新增临时人员</el-button>
				...
				<div class="bu-right" v-if="integrityArchives_btn_inspection && !skipVisible">
					<el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_solidification" class="filter-item" style="margin-left: 10px;" @click="solidificationHandle"
						type="primary" icon="edit">固化任务设置</el-button>
					<el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_temporary" class="filter-item" style="margin-left: 10px;" @click="temporaryHandle"
						type="primary" icon="edit">新增临时人员</el-button>
					<el-button v-if="integrityArchives_btn_inspection && integrityArchives_btn_personnelpool" class="filter-item" type="primary" v-waves icon="search" @click="personnelHandle()">人员库管理</el-button>
					<el-button class="filter-item" type="primary" v-waves icon="search" @click="skipHandle()">我的任务</el-button>
				...
				<!-- <div v-if="!integrityArchives_btn_inspection && !integrityArchives_btn_first" class="bu-right">
					<el-button class="filter-item" style="margin-left: 10px;" @click="solidificationPutHandle('oneself')"
						type="primary" icon="edit">++自行提报</el-button>
				</div> -->
			</div>
			<!-- <div v-if="integrityArchives_btn_inspection && integrityArchives_btn_history" class="bu-right">
				<el-button class="filter-item" type="text" v-waves icon="search" @click="historyHandle()">固化任务历史记录</el-button>
			</div> -->
			<!-- <div style="height: 1px; background-color: black;"></div> -->
		</div>
		<!-- 纪检 -->
		<div v-if="integrityArchives_btn_inspection && !skipVisible" class="tree-info tree-roll-containers" ref="tree-info">
			<tree @refreshData="treeHandle" ref="tree"></tree>
		</div>
		<!-- 非纪检 -->
		<div v-if="tableVisible" class="page-info" ref="pageInfo" :style="integrityArchives_btn_inspection && !skipVisible?'margin-left:310px' : 'margin-left:0px'">
			<el-table :height="tableHeight" :key='tabPosition' :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column min-width="350" align="center" label="任务表格" :show-overflow-tooltip="true">
					<template slot-scope="scope">
						<div style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" class="page-info-taskTitle">
							...
							<el-table height="400" style="width: 100%" :data="scope.row.formInfo" border class="page-info-taskTitle-table">
							...
							<el-button type="text" slot="reference" @click="newProjectPopoverHandle">
						 ...
						 <!-- v-else -->
						<!-- <span v-for="(item,index) in scope.row.formInfo" :key="index" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
				...
				<el-table-column align="center" label="操作" min-width="200"> 
					<template slot-scope="scope">
						<el-button v-if="scope.row.status !== '2' && integrityArchives_btn_detail" type="text" size="small" @click="solidificationPutHandle('detail', 'detail',scope.row)">查看
						</el-button>
						...
						<el-button v-if="tabPosition == '我的提报' && (scope.row.status == '2' || scope.row.status == '3')" size="small" type="text" @click="solidificationPutHandle('add', 'add',scope.row)">去提报
						</el-button>
						<el-button v-if="tabPosition == '我的审核' && scope.row.status == '1'" size="small" type="text" @click="solidificationPutHandle('check', 'detail',scope.row,'','','examine')">去审核
						</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<!-- 纪检 -->
		<!-- <div v-if="integrityArchives_btn_inspection && !skipVisible" class="page-info" ref="pageInfo" style="margin-left:310px">
			<el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
				...
				<el-table-column fixed="right" align="center" label="操作" min-width="100">
					<template slot-scope="scope">
						<el-button v-if="integrityArchives_btn_detail" type="text" size="small" @click="solidificationPutHandle('detail', 'detail',scope.row)">查看
						</el-button>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			newProjectPopoverHandle() {
				// this.$refs.newProjectPopover.updatePopper();
				// this.$refs[toSrting(id)].updatePopper();
				// this.$nextTick(() => {
				// 	this.$refs.newProjectPopover.updatePopper();
				// })
				this.$nextTick().then(() => {
			...
			// 添加审批
			handleComments() {
                this.approveForm.auditAdvice = '';
                this.approveForm.auditStatus = '';
			...
			handleSelectionChange(val) {
				...
				this.approveForm.individualTaskIds = this.multipleSelection.map( item => { return item.id } )
			},
			radioChange() {
				this.listQuery =  {
					page: 1,
            ...
            tableLayout(option) {
                const {searchRefName = 'searchSection', subtractVal = 270} = option || {}
...
<style lang="scss" scoped>
	...
	// .page-info-taskTitle-table {
	// 	overflow: auto;
	// }

	// .page-info-taskTitle:hover {
	// 	color: #bc0000;
	// }
