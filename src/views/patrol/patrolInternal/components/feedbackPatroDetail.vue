	...
	<div class="tier-page content">
		<div class="content-body">
			<el-form :model="form" ref="form" size="mini" style="font-size: 15px;" label-position="left" label-width="120px">
                <el-form-item label="巡查反馈文件：">
					<div v-if="form.feedbackFiles && form.feedbackFiles.length" class="file-roll-containers">
						<div class="file-info" v-for="(item, index) in form.feedbackFiles" :key="index">
							<div class="file-img">
                ...
                <el-form-item label="提报时间：">
					<div class="form-text">{{form.submitTime}}</div>
				</el-form-item>
				<el-form-item label="被巡查单位：" prop="name">
					<div class="form-text">{{form.beInspectionName}}</div>
				</el-form-item>
                <el-form-item label="意见和要求:">
                    <div class="form-text">{{form.opinionRequire}}</div>
                </el-form-item>
			</el-form>
		</div>
	</div>
			...
			init() {
				this.$nextTick(() => {
					if (this.$route.query.id) {
						getFeedbackObj(this.$route.query.id)
							.then(response => {
