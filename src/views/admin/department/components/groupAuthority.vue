<template>
	<el-row :gutter="10">
		...
		<el-col :span="8" style='margin-top:10px;'>
			...
			<el-tree class="filter-tree" check-strictly :node-key="getTreeNodeKey" :data="treeData" show-checkbox
...
<script>
	...
	export default {
		...
		methods: {
			getDeptElementList() {
				getDeptElement(this.deptId).then(data => {
			...
			handleSelectAllChange(val) {
				...
				// editDeptElement(this.deptId, ids);
			},
			handleSelectionChange(val, row) {
				...
				// editDeptElement(this.deptId, [row.id]);
			},
			// 查重
			unique(id) {
				let ary = [];
			...
			update() {
				...
				editDeptElement(this.deptId, this.idsSelected);
				editDeptMenu(this.deptId, ids).then(() => {
					this.$notify({
						title: '成功',
						message: '操作成功',
						type: 'success',
			...
			initAuthoritys() {
				getDeptMenu(this.deptId).then(data => {
