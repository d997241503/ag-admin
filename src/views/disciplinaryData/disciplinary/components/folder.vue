			...
			<el-tree :data="data" draggable node-key="id" :expand-on-click-node="false" @node-click="handlePartyBranch" :props="defaultProps" highlight-current>
				<span class="custom-tree-node show-hide" slot-scope="{ node, data }" tabindex="1">
					...
					<span style="display:none">
						<i class="el-icon-plus" v-if="integrityArchives_btn_informationnameadd" @click="append(data,node)" />
						<i class="el-icon-edit" v-if="integrityArchives_btn_informationedit" @click="rename(data,node)" style="margin-left: 6px;" />
						<i class="el-icon-delete" v-if="integrityArchives_btn_informationdelete" @click="remove(data,node)" style="margin-left: 6px;" />
					</span>
				</span>
			</el-tree>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取单位档案树结构
			getTreeInfo() {
				getUnitarchivesTree({ value: this.value, orgId: this.form.orgId })
					.then(res => {
						if (res && res.status === 200) {
