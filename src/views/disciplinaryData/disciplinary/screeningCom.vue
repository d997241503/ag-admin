<template>
	<div class="app-container calendar-list-container nav-content" @click="OptionCardClose">
		...
		<div class="page-info-img" ref="pageInfo" style="margin-left: 310px;">
            <el-button v-if="integrityArchives_btn_riskpointsfilesadd" type="primary" @click="handleEdit()" style="float: right;margin-right: -70px;margin-top: -30px;">{{form.fileUrls.length ? '编辑' : '添加'}}</el-button>
	...
	export default {
		...
		methods: {
			...
			archivesfileHandle(formName) {
				...
				// 	if (valid) {
						updateUnitriskpointsfileObj(this.form)
							.then((res) => {
								if (res && res.status === 200) {
									this.$notify({
										title: '成功',
